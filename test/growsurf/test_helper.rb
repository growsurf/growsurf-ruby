...
class Growsurf::Test::SingletonClient < Growsurf::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: Growsurf::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class Growsurf::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @growsurf = Growsurf::Test::SingletonClient.instance
  end
